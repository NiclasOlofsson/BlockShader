...
float smoothrand2d(float horizont, float forward)
{
	...
	float smoothy = smoothstep(0.45, 0.55, frac(forward));
...
void main(in PS_Input PSInput, out PS_Output PSOutput)
{
	...
	// Set water
	//diffuse = lerp(watercolor, farwatercolor, max(0.9, min(1.0, PSInput.color.a)));
	...
	if (PSInput.uv1.y >= 0.5)
	{
		...
		{
			diffuse = float4(0.5, 0.5, 0.5, 0.5) * uvs;
		}
	}
	...
	float4 cloud = FullCloudMap(PositionTransform(reflected_pos * 1.3, 500.0), 0.3, PSInput.color);

	// Uncomment to enable cloud reflections in water. Cool effect,
	// but not so good looking.
	//diffuse = lerp(diffuse, cloud, (cloud.a * PSInput.uv1.y));

	diffuse.a = pow(view_angle / (1), 2);

	float4 suncolor = GetSunColor(reflected_pos * 2.3, uvs.r);
	...
	//			float fadeInE = (PSInput.uv1.g - (shwE - fSwE.r))*fSwE.g;
